...
from .pipelines import LoadRddAnnotations
...
class RddDataset(Dataset):
    ...
    CLASSES = ('bg','road','bonnet','line_mark','crossing','other_lane','crack','pothole','manhole')

    PALETTE = [[0,0,0],[254,255,189], [203,192,255], [83,50,250], [102,255,102], [245,61,184],
               [200,25,255], [150,192,255], [255, 100, 0]]
    ...
    def __init__(self,
        ...
        self.gt_seg_map_loader = LoadRddAnnotations(
            reduce_zero_label=reduce_zero_label, **gt_seg_map_loader_cfg)
    ...
    def load_annotations(self):
        ...
        for line in fp:
            ...
            if self.data_root is not None:
                ...
                rect_path = osp.join(self.data_root, rect_path)
                lane_path = osp.join(self.data_root, lane_path)

            img_info = dict(filename=img_path)
            img_info['ann'] = dict(seg_map=dmg_path, seg_map2=lane_path, rect=rect_path)
    ...
    def convert_logits_to_label(self, seg_logit):
        ...
        # pred = torch.zeros(seg_logit.size(0), seg_logit.size(2), seg_logit.size(3)).cuda()

        road_logit = seg_logit[:, :ROAD_CLASS_NUM,:,:]
        damage_logit = seg_logit[:, ROAD_CLASS_NUM:,:,:]

        rlab = road_logit.argmax(dim=1)
        dlab = damage_logit.argmax(dim=1) + ROAD_CLASS_NUM - 1
        
        rmask = ((rlab == 1) | (rlab > 2) & (dlab != ROAD_CLASS_NUM-1)) 

        pred = rlab 
        pred[rmask] = dlab[rmask]
